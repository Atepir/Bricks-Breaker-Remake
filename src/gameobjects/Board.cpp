...
Board<eMapType::Basic>::Board<eMapType::Basic>()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            mBricks.push_back(std::make_shared<Brick>(Point(BOARD_PADDING + i * (BRICK_WIDTH), BOARD_PADDING + j * (BRICK_HEIGHT)), BRICK_WIDTH, BRICK_HEIGHT, BrickType::NORMAL, INITIAL_BRICK_HEALTH));
        }
    }
}
